...
public class SAXWriter implements XMLReader {
    ...
    public AttributesImpl startPrefixMapping(Element element, NamespaceStack namespaceStack) throws SAXException {
        ...
        for (Namespace next : element.declaredNamespaces()) {
            if (!isIgnoreableNamespace(next, namespaceStack)) {
                namespaceStack.push(next);
                this.contentHandler.startPrefixMapping(next.getPrefix(), next.getURI());
                attributesImpl = addNamespaceAttribute(attributesImpl, next);
            }
        }
    ...
    public AttributesImpl addNamespaceAttribute(AttributesImpl attributesImpl, Namespace namespace) {
        ...
        if (this.declareNamespaceAttributes) {
            ...
            attributesImpl.addAttribute(BuildConfig.FLAVOR, prefix, str, "CDATA", namespace.getURI());
        }
        return attributesImpl;
    }
